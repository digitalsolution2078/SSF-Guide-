...
export default async function AdminBlogPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10">      <h1 className="mt-2 text-2xl font-medium text-ink-900">📰 Content — Blog Manager</h1>
      ...
      <form action={createPostAction} className="mt-6 space-y-3 rounded-xl border border-ink-200 bg-white p-5">
        <p className="font-semibold text-ink-800">नयाँ post</p>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
          <label className="block text-sm font-semibold text-ink-700 sm:col-span-3">
            शीर्षक
            <input name="title" required className="mt-1 w-full rounded-lg border border-ink-300 px-3 py-2" />
          </label>
          <label className="block text-sm font-semibold text-ink-700">
            Emoji
            <input name="emoji" defaultValue="📝" maxLength={4} className="mt-1 w-full rounded-lg border border-ink-300 px-3 py-2" />
          </label>
        </div>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <label className="block text-sm font-semibold text-ink-700">
            Category
            <input name="category" defaultValue="समाचार" className="mt-1 w-full rounded-lg border border-ink-300 px-3 py-2" />
          </label>
          <label className="block text-sm font-semibold text-ink-700">
            Slug (वैकल्पिक, English)
            <input name="slug" placeholder="ssf-new-update" className="mt-1 w-full rounded-lg border border-ink-300 px-3 py-2" />
          </label>
        </div>
        <label className="block text-sm font-semibold text-ink-700">
          Excerpt (छोटो सार)
          <textarea name="excerpt" rows={2} required className="mt-1 w-full rounded-lg border border-ink-300 px-3 py-2" />
        </label>
        <label className="block text-sm font-semibold text-ink-700">
          Body
          <textarea name="body" rows={8} required className="mt-1 w-full rounded-lg border border-ink-300 px-3 py-2 font-mono text-sm" />
        </label>
      ...
      <div className="mt-8">
        <p className="mb-3 text-sm font-semibold text-ink-800">Blog posts ({posts.length})</p>
        {posts.length === 0 ? (
          <p className="rounded-xl border border-ink-200 bg-white p-6 text-center text-ink-500">अहिलेसम्म कुनै post छैन।</p>
        ) : (
          <ul className="space-y-3">
            {posts.map((p) => (
              <li key={p.id} className="flex items-start justify-between gap-3 rounded-xl border border-ink-200 bg-white p-4">
                <div>
                  <p className="font-medium text-ink-900">{p.emoji} {p.title}</p>
                  <p className="mt-1 text-xs text-ink-400">{p.status} · {p.category} · /blog/{p.slug}</p>
                </div>
